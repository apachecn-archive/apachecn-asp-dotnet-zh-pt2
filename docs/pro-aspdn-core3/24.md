...
数据库将作为应用启动的一部分被植入。一旦 ASP.NET 核心正在运行，使用 web 浏览器请求`http://localhost:5000/cities`，这将产生如图 [24-1](#Fig1) 所示的响应。
...
应用通常需要在视图中嵌入与应用的主要目的无关的内容。常见的例子包括站点导航工具和身份验证面板，让用户无需访问单独的页面即可登录。
