...
重启 ASP.NET 核心并使用浏览器请求`http://localhost:5000/controllers`。点击提交按钮将表单发送到应用，您将在结果中看到一个新的项目，如图 [27-9](#Fig9) 所示。
...
结果中显示的`_RequestVerificationToken`表单值是一个安全特性，由`FormTagHelper`用来防止跨站点请求伪造。跨站点请求伪造(CSRF)通过利用用户请求通常被验证的方式来利用 web 应用。大多数 web 应用——包括那些使用 ASP.NET 核心创建的应用——使用 cookies 来识别哪些请求与特定的会话相关，用户身份通常与该会话相关联。
...
默认情况下，防伪功能依赖于 ASP.NET 核心应用能够在 HTML 表单中包含一个元素，在提交表单时浏览器会发回该元素。这对 JavaScript 客户机不起作用，因为 ASP.NET 核心应用提供数据而不是 HTML，所以没有办法插入隐藏元素并在未来的请求中接收它。
...
清单 [27-31](#PC46) 显示了 ASP.NET 核心应用配置用于 JavaScript 客户端的防伪特性所需的更改。
