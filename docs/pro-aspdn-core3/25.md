...
事实上，问题更严重，因为视图导入文件中的`@addTagHelper`指令意味着示例标签助手应用于控制器和 Razor 页面呈现的任何视图中使用的所有`tr`元素。例如用浏览器请求`http://localhost:5000/cities`，你会看到城市Razor页面响应中的`tr`元素也被转换了，如图 [25-3](#Fig3) 所示。
