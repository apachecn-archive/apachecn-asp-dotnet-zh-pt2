...
运行 web 应用，URL 现在完全符合我的要求(图 [2-12](#Fig12) )。如果我想告诉 ASP.NET ID 参数是可选的，我也可以通过指定`"{videoId?:int}".`在`@page`指令中指定这一点，但是无论我在这里做什么，ASP.NET 核心知道 URL 的第三段将指定一个名为`videoId`的参数。
