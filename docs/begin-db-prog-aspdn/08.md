...
在前面的小节中，您使用 Blazor 服务器端托管模型开发了 Employee Manager。尽管应用能够执行 CRUD 操作，但它缺乏用户认证。就用户认证而言，Blazor 服务器端应用可以利用 ASP.NET 核心身份，就像 ASP.NET 核心 MVC 和 ASP.NET 核心 Razor Pages 应用一样。
