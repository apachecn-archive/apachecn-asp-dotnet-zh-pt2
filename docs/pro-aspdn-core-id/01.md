...
在本书的第 1 部分，我将向您展示如何将 ASP.NET 核心身份应用到 ASP.NET 核心项目中。我将向您展示如何设置和配置Identity，如何使用内置的Identity UI 包来管理用户帐户，以及如何使用 API 来创建自定义工作流。在本书的这一部分结束时，你将准备好在你的项目中使用 ASP.NET 核心身份。
...
如上所述，这本书是为有经验的 ASP.NET 核心开发人员编写的，他们希望了解 ASP.NET 核心身份并将其应用到他们的 ASP.NET 核心项目中。它没有解释 web 应用或编程的基础知识。它没有描述 ASP.NET 核心平台，除了它与Identity如何工作有关。
