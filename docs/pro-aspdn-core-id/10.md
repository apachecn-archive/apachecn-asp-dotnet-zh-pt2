...
`ClaimsPrincipal`类定义了`Identities`属性，该属性返回一系列代表用户 Identity 的`ClaimsIdentity`对象。`ClaimsIdentity`类定义了`Claims`属性，该属性返回一系列`Claim`对象。我将在后面的示例和第 2 部分中描述其他特性，但是这个基本的属性集足以显示用户登录应用时由 ASP.NET Core 身份生成的声明集。
