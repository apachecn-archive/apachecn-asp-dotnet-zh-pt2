...
在第 1 部分中，我重点解释了如何使用标识。在这本书的这一部分，我解释了Identity是如何工作的，回顾了主要特征并描述了幕后发生的事情。我创建自定义用户角色存储，使用自定义用户和角色类，并实现 ASP.NET 核心身份使用的许多接口。

在这一章中，我创建了一个简单的示例项目，将在后面的章节中使用。我用这个项目来解释 ASP.NET 核心是如何处理认证和授权的，以及Identity是如何建立在这些特性之上的。
...
当我需要简单和独立的东西时，我使用Razor页。对于更复杂的功能，我更喜欢使用 MVC 框架。创建`ExampleApp/Controllers`文件夹，并添加一个名为`HomeController.cs`的类文件，代码如清单 [13-8](#PC8) 所示。
