...
在前一章中，我重点介绍了 ASP.NET Core 请求管道以及认证请求和授权访问端点的中间件组件。在这一章中，我将深入探讨授权特性，解释限制访问的不同方式。这些都是 ASP.NET Core 提供的功能，我在描述它们时没有使用 ASP.NET Core 身份。表 [15-1](#Tab1) 将 ASP.NET Core 授权特性放在上下文中。
...
清单 [15-2](#PC2) 中的代码基于内置 ASP.NET Core 中间件授权请求的方式。ASP.NET Core 和 ASP.NET Core 身份是开源的，探索源代码是理解特性是如何实现的好方法。
