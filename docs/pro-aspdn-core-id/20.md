...
重启 ASP.NET 核心并请求`http://localhost:5000/signin`。使用密码`MySecret1$`以 bob@example.com 的 Identity 登录应用，您将看到如图 [20-3](#Fig3) 所示的错误消息，显示该消息是因为鲍勃没有被分配`Administrator`角色，也没有`UserConfirmed`声明。
