...
回答
 ...
 |
| --- | --- |
| 这是什么？ | Identity UI 包是微软提供的一组 Razor 页面和支持类，用于在 ASP.NET Core 项目中快速启动 ASP.NET Core 身份的使用。 |
...
使用 PowerShell 命令提示符运行清单`IdentityApp`文件夹中的 [4-3](#PC3) 所示的命令，将 ASP.NET Core 身份包添加到项目中。
...
下一步是配置应用，以设置用于存储用户数据的数据库，并配置 ASP.NET Core 身份。将清单 [4-6](#PC6) 中显示的语句添加到`Startup`类中。
...
清单 [4-6](#PC6) 中的另一个新语句建立了 ASP.NET Core 身份。第一部分调用`AddDefaultIdentity`方法，就像这样:
...
在本章中，我解释了如何将 Identity 和 Identity UI 包添加到项目中。我向您展示了如何准备 Identity 数据库，解释了如何覆盖单个文件以创建一致的布局，并描述了 Identity UI 包提供的默认工作流。在下一章，我将向您展示如何配置 ASP.NET Core 身份。*