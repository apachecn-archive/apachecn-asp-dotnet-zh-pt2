...
用于创建一个`AuthorizationPolicy`对象的两个参数是一系列需求和一系列认证方案。我在前面的例子中使用了一个空数组，它不限制将要使用的模式，但是在支持多种身份验证模式并且需要限制通过其中一部分身份验证的用户访问的应用中，这是一个有用的特性。在清单 [15-14](#PC20) 中，我添加了一个定制授权测试中间件将使用的新方案。
...
这个类实现了`IActionModelConvention`接口，它是应用模型特性的一部分，允许改变动作方法。构造函数接受一个控制器名，还可以选择接受应该应用策略的操作。授权策略、可接受的角色集和可接受的身份验证方案都有可选的参数。如果没有提供角色或策略，将使用默认策略。如果没有指定动作方法，则策略将应用于控制器定义的所有动作。
...
在本章中，我解释了 ASP.NET 核心如何使用策略授权请求。授权策略是身份所依赖的关键构造块，尽管很少有应用需要直接使用它们。在下一章中，我将身份引入到示例项目中，并创建一个自定义用户存储。