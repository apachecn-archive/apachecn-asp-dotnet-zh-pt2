# 1.了解 Angular 和 ASP.NET Core MVC
...
这本书解释了如何在 ASP.NET Core MVC 项目中使用 Angular。我将演示如何创建一个包含 Angular 和 ASP.NET Core MVC 的 Visual Studio 或 Visual Studio 代码项目，并向您展示如何让它们协同工作。我将向您展示如何使用实体框架核心来存储应用数据，以及如何使用 ASP.NET Core 身份来验证和授权用户。每个 ASP.NET Core 包都增加了它的复杂性，我将向您展示如何管理这些来为 Angular 提供功能。
